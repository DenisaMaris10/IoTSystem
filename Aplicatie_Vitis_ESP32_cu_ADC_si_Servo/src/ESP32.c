...
int initATCtrl(u32 UART_DEVICE_BASEADDR, Uart * devicePtr) {
    int Status;
	xil_printf("Inside of initATCtrl\n\r");
    Status = XUartLite_Initialize(devicePtr, UART_DEVICE_BASEADDR);
...
int testStartup(Uart *devicePtr){
    u8 tx[] = "AT";
    while(XUartLite_IsSending(devicePtr));
...
int TCPsend(Uart * devicePtr, char * data, int length) {
    u8 tx_buf[50];
